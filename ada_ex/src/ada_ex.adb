...
procedure Ada_Ex is
   ...
   x          : Integer := 1;
   --  Something like HelloWorld : Integer := "Hello World!"; will not work
   --  because the type of the variable is integer and the value is a
   --  string. Showing how Ada is a strongly typed language.
   ...
   --  range constraints
   --  y is constrained to be between 1 and 10, inclusive
   y : Integer range 1 .. 10 := 5;

   --  Functions
   --  A function always returns a value.
   --  syntax: function Name (Param : Type) return Return_Type is
   --  ... begin ... return ...; end Name;
   ...
   function Increment
     (Value : Integer; Step : Integer := 1) return Integer
   is
   begin
      return Value + Step;
   end Increment;
...
begin
   ...
   --  Schedule the next execution exactly 5 seconds from
   --  the previous scheduled execution.
   Time_1 := Ada.Calendar.Clock; --  Current time before the delay
   Ada.Text_IO.Put_Line
     ("Hello World! First " & Ada.Calendar.Formatting.Image (Time_1));

   --  Wait until the scheduled time
   Next_Time :=
     Ada.Real_Time."+" (Ada.Real_Time.Clock, Ada.Real_Time.Seconds (5));
   delay until Next_Time;
   Time_2 := Ada.Calendar.Clock; --  Current time after the delay
   Ada.Text_IO.Put_Line
     ("Hello World! Second " & Ada.Calendar.Formatting.Image (Time_2));
   ...
   for NEXT in 1 .. Ada.Command_Line.Argument_Count loop
      --  A declare block gives us a place to declare per-iteration
      --  variables.
      declare
   ...
   Ada.Text_IO.Put_Line
     ("Increment(10) =" & Integer'Image (Increment (10)));

   --  Increment with an explicit step.
   Ada.Text_IO.Put_Line
     ("Increment(10, 5) =" & Integer'Image (Increment (10, 5)));
